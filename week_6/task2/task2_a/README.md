...
Программа состоит из двух файлов: `DictionaryConverter` и `Main`. Для запуска программы воспользуйтесь файлом `Main`.

В файле `DictionaryConverter` находится метод `convertToLatinEnglish`, который преобразует введённую пару значений в необходимый по заданию вид.

Для преобразования словаря используется `TreeMap`, который сортирует словарь по ключам. 

В файле `Main` осуществляется ввод количество слов и словаря, которое необходимо преобразовать, вызов класса для анализа введённого значения, а также вывод результата.